...
router.route('/').get(async (req, res) => {
    try {
        ...
        const totalReports = await REPORT.find(finishedStatus).countDocuments()

        let organCount = await REPORT.aggregate([
            {
                $match: finishedStatus,
            },
            {
                $project: {
                    lastReport: {
...
router.route('/:departmentID').get(async (req, res) => {
    try {
        ...
        const totalReports = await REPORT.find(matchConditions).countDocuments()
