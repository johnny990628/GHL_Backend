...
function createTable() {
        ...
        for (var j = 0; j < Patient.Study[i].SeriesAmount; j++) {
            ...
            for (var k = 0; k < Patient.Study[i].Series[j].SopAmount; k++) {
                ...
                if (Patient.Study[i].Series[j].Sop[k].StudyTime != undefined)
                    StudyTime_list.push(
                        ('' + Patient.Study[i].Series[j].Sop[k].StudyTime).replace(/^(\d{2})(\d\d)(\d\d)/, '$1:$2:$3').substr(0, 8)
                    )
    ...
    for (var i = 1; i <= rows; i++) {
        ...
        if (!Modality_list[i - 1].includes('SM')) row.alt = ConfigLog.QIDO.target + '?' + str
        ...
        row.onclick = function () {
            ...
            cell.innerHTML = "<b>Open This Study </b><img src='../image/icon/goto.png'></img>"
            if (!this.Modality.includes('SM')) cell.alt = ConfigLog.QIDO.target + '?' + 'StudyInstanceUID=' + this.Study.StudyUID
            else cell.alt = ConfigLog.QIDO.targetSM + '?' + 'StudyInstanceUID=' + this.Study.StudyUID
            ...
            for (var c = 0; c < this.Study.Series.length; c++) {
                ...
                var row = Table2.insertRow(Series_c_count + 2)
                var cell = row.insertCell(0)
                cell.innerHTML = "<b>Open This Series </b><img src='../image/icon/goto.png'></img>"
                if (!this.Modality.includes('SM'))
                    cell.alt =
                        ConfigLog.QIDO.target +
                        '?' +
            ...
            Table.style.marginTop =
                parseFloat(Table.style.marginTop) - Table2.getClientRects()[0].height + this.getClientRects()[0].height * 1 + 'px'
